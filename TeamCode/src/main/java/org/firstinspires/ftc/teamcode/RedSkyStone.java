...
public class RedSkyStone extends BaseRobot {
    private int stage = 0;
    private double distance_mec1 = 0.0;     // distance moved to find the 1st black skystone; Pos for Red/LEFT, Neg for Blue/RIGHT
    ...
    public void loop() {
        ...
        switch (stage) {
            case 0:          // SLOWLY Forward 45 inches or touches the skystone
                ...
                //}
                if (auto_drive(0.75, 44.0)) {
                    reset_drive_encoders();
                    stage++;
                }
                break;
            case 1:          // SLOWLY Strafe LEFT up to 75 inches until a black skystone is found
...
// }
    ...
    //            if (close_servos())
            ...
            case 4:         // Back slowly to avoid hitting the skybridge during mecanum
                if (auto_drive(-0.75, 10.0)) {
                    reset_drive_encoders();
                    stage++;
                }
                break;
            case 5:         // Move RIGHT 80 inches + distance moved to drop the black skystone
                if (auto_mecanum(1.0, (86.0 + distance_mec1))) {    // Strafe RIGHT
                    reset_drive_encoders();
                    stage++;
                }
                break;
    ...
    //            open_servos();
            ...
            case 8:
                ...
                break;
            case 9:         // Strafe LEFT 16 inches to PARK
                if (auto_mecanum(-1.0, 19.0)) { // Strafe LEFT to PARK
                    reset_drive_encoders();
        ...
        if (DEBUG) telemetry.addData("Red Skystone: ", stage);
        super.loop();
    }
}
